...
class VisualService:
    ...
    # ----------------------------------------------------------------- #
    def _youtube_thumbnail_frames(
        self, url: str, work_dir: str
    ) -> list[tuple[float, str]]:
        """Cloud-safe YouTube frames via i.ytimg.com (no yt-dlp download)."""
        video_id = extract_youtube_id(url)
        if not video_id:
            return []

        max_frames = max(1, settings.visual_max_frames)
        paths = download_youtube_frames_for_url(url, work_dir, max_frames=max_frames)
        if not paths:
            return []
